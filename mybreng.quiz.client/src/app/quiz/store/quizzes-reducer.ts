...
export const quizzesReducer = createReducer(
    ...
    on(quizzesActions.setLoading, (state, { loading }) => {
        let newCounter = state.loadingCounter;
        if(loading) {
            newCounter = state.loadingCounter + 1;
        } else if(state.loadingCounter > 0) {
            newCounter = state.loadingCounter - 1;
        }
        return { ...state, loadingCounter: newCounter };
    }),
    ...
    on(quizzesActions.applyFilter, (state, { filter }) => ({
        ...state,
        listFilter: filter
    })),
